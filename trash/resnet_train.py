...
class CNN(nn.Module):
  def __init__(self):
    ...
    self.ArcFace_layer = ArcMarginProduct(128,10178)
...
if __name__ == '__main__':
  ...
  for i, fname in enumerate(glob.glob("./dataset_csv/*.csv")):
    input_file_path = './dataset_csv/dataset0.csv'
    imgDataset = my_dataset.MyDataset(fname, transform=transforms.Compose([transforms.Resize(224), transforms.ToTensor()]))
  ...
  # print(torch.load('model_weight.pth'))
    net.load_state_dict(torch.load('model/resnet_model_weight.pth'))
    ...
    for epoch in range(1, 1+1):
      torch.save(net.state_dict(), "./model/resnet_model_weight.pth")
