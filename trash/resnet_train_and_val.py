...
class CNN(nn.Module):
  def __init__(self):
    ...
    self.ArcFace_layer = ArcMarginProduct(128,300)
...
if __name__ == '__main__':
  ...
  input_file_path = './utils/dataset.csv'
  imgDataset = my_dataset.MyDataset(input_file_path, transform=transforms.Compose([transforms.Resize(224), transforms.ToTensor()]))
  ...
  test_loader = torch.utils.data.DataLoader(test_data, batch_size=32, shuffle=False,num_workers=0, pin_memory=True)
  ...
  net.load_state_dict(torch.load('newmodel/resnet_model_weight.pth'))
  ...
  for epoch in range(1, 50+1):
    torch.save(net.state_dict(), "./newmodel/resnet_model_weight.pth")
